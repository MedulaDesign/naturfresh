...
class lorem {
	...
	static function get ($lenght=100,$start='',$end='.') {
		$words = self::words() ;
		$total = count($words) ;
		global $ipsum_offset ;
	...
	static function text ($total=5,$lenght=10,$sep="\n\n") {
		$text = array() ;
		for ( $i=0 ; $i < $total ; $i++ ) {
	...
	static function html ($lenght=100,$tags='strong/em/a') {
		$tags = explode('/',$tags) ;
		$text = self::get($lenght,'','') ;
		return self::html_rec($text,$tags) ; }

	static function html_rec ($text,$tags) {
		if ($tags) {
			$text = explode(' ',$text) ;
			$count = count($text) ;
			$index = 0 ;
	...
	static function html_tag ($text,$tag) {
		switch ($tag) {
			case 'a' : return '<a href="#">'.$text.'</a>' ;
			default : return '<'.$tag.'>'.$text.'</'.$tag.'>' ; } }

	static function p ($lenght=100,$tags='strong/em/a') {
		return '<p>'.self::html($lenght,$tags).'</p>' ; }

	static function h ($size=3,$lenght=10,$tags='strong/em/a') {
		return '<h'.$size.'>'.self::html($lenght,$tags).'</h'.$size.'>' ; }

	static function lists ($type='ol',$total=5,$lenght=10,$tags='strong/em/a') {
		$items = array() ;
		for ( $i=0 ; $i < $total ; $i++ ) {
			$items[] = '<li>'.self::html($lenght,$tags).'</li>' ; }
		return '<'.$type.'>'.implode("\n",$items).'</'.$type.'>' ; }

	static function ol ($total=5,$lenght=10,$tags='strong/em/a') {
		return self::lists('ol',$total,$lenght,$tags) ; }

	static function ul ($total=5,$lenght=10,$tags='strong/em/a') {
		return self::lists('ul',$total,$lenght,$tags) ; }

	static function rand ($c1,$c2,$t1,$t2,$f1,$f2) {
